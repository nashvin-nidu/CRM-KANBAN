...
return 4;
    ...
    const stageCounts = stages.map((stage, idx) => {
        ...
        const value = cumulativeLeads.reduce(
            (sum, l) => sum + (l.value || 0),
            0,
        );
...
const teamPerformance = computed(() => {
    ...
    const performance = salesTeam.value.map((member) => {
        ...
        const activeLeads = memberLeads.filter(
...
return null;
    ...
    const lowestWorkload = [...list].sort(
        (a, b) => a.activeLeads - b.activeLeads,
    )[0];

    // Highest historical win rate
    const highestConversion = [...list].sort(
...
const conversionChartSvg = computed(() => {
    ...
    const bars = data.map((d, idx) => {
        const barWidth = 24;
        const xCenter =
            paddingLeft + idx * (chartWidth / (data.length - 1 || 1));
        const x = xCenter - barWidth / 2;
        const barHeight = Math.max(
            (d.conversionRate / maxVal) * chartHeight,
            3,
...
<template>
    ...
    <div class="flex h-full flex-1 flex-col gap-6 overflow-x-auto p-6">
        <!-- Header Section -->
        <div
            class="flex shrink-0 flex-col gap-4 md:flex-row md:items-center md:justify-between"
        >
            <div>
                <h1
                ...
                <p class="mt-1 max-w-xl text-sm text-muted-foreground">
                    Real-time performance, conversions, and intelligent
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <Select v-model="userFilter" v-if="isAdmin">
                    <SelectTrigger
                        class="flex h-9 w-fit min-w-[140px] items-center justify-between gap-2 rounded-md border border-sidebar-border bg-card/60 px-3 text-xs shadow-2xs transition-colors hover:bg-card/90 focus:ring-0 focus:ring-offset-0"
                    >
                ...
                >
                    ...
                    <SelectContent class="text-xs">
                        ...
                        <SelectItem
                            v-for="source in availableSources.filter(
                                (s) => s !== 'all',
                            )"
                            :key="source"
                            :value="source"
                ...
                <Button
                    size="sm"
                    as-child
                    class="h-9 text-xs font-medium shadow-2xs transition-transform active:scale-98"
                >
                    <Link :href="kanban()">
                        Manage Pipeline
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <!-- KPI 1: Total Leads -->
            <Card
                class="relative overflow-hidden border border-sidebar-border/70 bg-card/40 shadow-sm backdrop-blur-xs transition-all duration-300 hover:shadow-md dark:border-sidebar-border/80"
            >
                <CardHeader
                ...
                >
                    ...
                    >
                    <div
                        class="rounded-md bg-muted/60 p-2 text-muted-foreground"
                    >
                        <Users class="h-4 w-4" />
                    </div>
                </CardHeader>
                <CardContent class="pt-1">
                    ...
                    <div class="text-2xs mt-2 flex items-center gap-1.5">
                        <span
            ...
            >
                ...
                <CardContent class="pt-1">
                    <div
                        class="text-2xl font-bold tracking-tight text-foreground"
                    >
                        {{ formatNumber(kpis.qualifiedLeads) }}
                    </div>
                    <div class="text-2xs mt-2 flex items-center gap-1.5">
                        <span
                            class="inline-flex items-center gap-0.5 rounded-sm px-1.5 py-0.5 font-bold"
                            :class="[
                                trends.qualified.isPositive
                                    ? 'bg-success/10 text-success'
                                    : 'bg-destructive/10 text-destructive',
                            ]"
                        >
                            <TrendingUp
                                class="h-3 w-3"
                                v-if="trends.qualified.isPositive"
                            />
                            <TrendingDown class="h-3 w-3" v-else />
            ...
            >
                ...
                >
                    ...
                    >
                    <div
                        class="rounded-md bg-emerald-500/10 p-2 text-emerald-600 dark:text-emerald-400"
                    >
                ...
                <CardContent class="pt-1">
                    <div
                        class="text-2xl font-bold tracking-tight text-foreground"
                    >
                        {{ formatNumber(kpis.wonDeals) }}
                    </div>
                    <div class="text-2xs mt-2 flex items-center gap-1.5">
                        <span
                            class="inline-flex items-center gap-0.5 rounded-sm px-1.5 py-0.5 font-bold"
                            :class="[
                                trends.won.isPositive
                                    ? 'bg-success/10 text-success'
                                    : 'bg-destructive/10 text-destructive',
                            ]"
                        >
            ...
            >
                ...
                <CardContent class="pt-1">
                    <div
                        class="truncate text-2xl font-bold tracking-tight text-foreground"
                    >
        ...
        <Card
            class="border border-sidebar-border/70 bg-card/25 shadow-sm dark:border-sidebar-border/80"
        >
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <CardDescription class="mt-1 text-xs">
                            Cumulative pipeline velocity and conversions
                            calculated from New to Won.
                        </CardDescription>
                    </div>
                    <Badge
                        variant="outline"
                        class="text-2xs bg-muted/30 px-2 py-0.5 font-semibold"
                    >
            ...
            <CardContent>
                <div
                    class="flex flex-col items-stretch gap-3 md:flex-row md:gap-2"
                >
                    <template
                    ...
                    >
                        <!-- Funnel Stage Card -->
                        <div
                            class="group relative flex flex-1 flex-col overflow-hidden rounded-xl border border-sidebar-border/60 bg-card/65 p-4 shadow-2xs transition-all duration-200 hover:border-foreground/15 dark:border-sidebar-border/80 dark:hover:border-foreground/30"
                        >
                            <!-- Background color overlay that visualizes conversion rate density -->
                            <div
                                class="absolute inset-x-0 bottom-0 h-1 bg-muted/30"
                            >
                                <div
                                    class="h-full bg-primary/75 transition-all duration-500 dark:bg-primary"
                                    :style="{
                            ...
                            <div class="mb-3 flex items-center justify-between">
                                ...
                                <Badge
                                    variant="secondary"
                                    class="text-2xs h-5 rounded-sm px-1.5 font-bold"
                                    :class="[
                                        idx === 0
                        ...
                        <div
                            ...
                            class="flex shrink-0 items-center justify-center px-2 py-1 text-muted-foreground/50 md:h-auto md:flex-col md:py-0"
                        >
                            <!-- Arrow desktop vs mobile -->
                            <ChevronRight class="hidden h-4 w-4 md:block" />
                            <span
                                class="text-2xs z-10 block rounded-sm bg-muted/65 px-1.5 py-0.5 text-center font-bold text-muted-foreground/75 md:-mt-1 md:-mb-1"
                            >
                                {{
                                    funnelStages[idx].count > 0
                                        ? Math.round(
                                              (funnelStages[idx + 1].count /
                                                  funnelStages[idx].count) *
                                                  100,
                                          )
                                        : 0
                                }}%
                            ...
                            <ChevronRight
                                class="mt-1 h-4 w-4 rotate-90 md:hidden md:rotate-0"
                            />
                        </div>
                    </template>
                </div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <!-- Revenue Trend Chart -->
            <Card
                class="relative border border-sidebar-border/70 bg-card/25 shadow-sm dark:border-sidebar-border/80"
            >
                <CardHeader class="pb-2">
                    <div class="flex items-center justify-between">
                        ...
                        >
                            {{
                                formatCurrency(
                                    revenueChartSvg.points[hoveredRevenueIndex]
                                        .value,
                                )
                            }}
                            ({{
                ...
                <CardContent class="pt-2">
                    <div class="relative h-[180px] w-full">
                        ...
                        >
                            ...
                            >
                                <line
                                    ...
                                    :y1="
                                        revenueChartSvg.paddingTop +
                                        (i - 1) *
                                            (revenueChartSvg.chartHeight / 2)
                                    "
                                    :x2="
                                        revenueChartSvg.width -
                                        revenueChartSvg.paddingRight
                                    "
                                    :y2="
                                        revenueChartSvg.paddingTop +
                                        (i - 1) *
                            ...
                            >
                                ...
                                <text
                                    :x="revenueChartSvg.paddingLeft - 8"
                                    :y="
                                        revenueChartSvg.height -
                                        revenueChartSvg.paddingBottom +
                                        3
                                    "
                                    text-anchor="end"
                                >
                        ...
                        <div
                            v-if="hoveredRevenueIndex !== null"
                            class="text-2xs pointer-events-none absolute z-10 space-y-0.5 rounded-md border border-sidebar-border bg-card/90 p-2.5 shadow-md transition-all duration-100 dark:bg-card/95"
                            :style="{
                        ...
                        >
                            <div class="font-bold text-foreground">
                                ...
                                {{ last6Months[hoveredRevenueIndex].year }}
                            </div>
                            <div
                                class="flex items-center gap-1 font-semibold text-success"
                            >
            ...
            >
                <CardHeader class="pb-2">
                    <div class="flex items-center justify-between">
                        ...
                        >
                            {{
                                conversionChartSvg.bars[hoveredConversionIndex]
                                    .value
                            }}% Win Rate
                        </span>
                        <span
                ...
                <CardContent class="pt-2">
                    <div class="relative h-[180px] w-full">
                        ...
                        >
                            ...
                            >
                                <line
                                    ...
                                    :y1="
                                        conversionChartSvg.paddingTop +
                                        (i - 1) *
                                            (conversionChartSvg.chartHeight / 2)
                                    "
                                    :x2="
                                        conversionChartSvg.width -
                                        conversionChartSvg.paddingRight
                                    "
                                    :y2="
                                        conversionChartSvg.paddingTop +
                                        (i - 1) *
                            ...
                            >
                                <text
                                    :x="conversionChartSvg.paddingLeft - 8"
                                    :y="conversionChartSvg.paddingTop + 3"
                                    text-anchor="end"
                                >
                                    100%
                                </text>
                                ...
                                <text
                                    :x="conversionChartSvg.paddingLeft - 8"
                                    :y="
                                        conversionChartSvg.height -
                                        conversionChartSvg.paddingBottom +
                                        3
                                    "
                                    text-anchor="end"
                                >
                        ...
                        >
                            <div class="font-bold text-foreground">
                                {{
                                    conversionChartSvg.bars[
                                        hoveredConversionIndex
                                    ].label
                                }}
                                {{ last6Months[hoveredConversionIndex].year }}
                            </div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <Card
                class="border border-sidebar-border/70 bg-card/25 shadow-sm lg:col-span-2 dark:border-sidebar-border/80"
            >
                <CardHeader class="pb-3">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription class="text-2xs">
                                Individual conversion rates and revenue
                                generated.
                            </CardDescription>
                        </div>
                        <Badge
                            variant="outline"
                            class="text-2xs border-emerald-500/20 bg-emerald-500/10 font-semibold text-emerald-600 dark:text-emerald-400"
                            v-if="topPerformer"
                        >
                ...
                <CardContent class="px-0 md:px-6">
                    <div class="overflow-x-auto">
                        <table class="w-full border-collapse text-left">
                            <thead>
                                <tr
                                    class="text-2xs border-b border-sidebar-border/60 tracking-wider text-muted-foreground uppercase"
                                >
                            ...
                            <tbody
                                class="divide-y divide-sidebar-border/40 text-xs"
                            >
                                <tr
                                    v-for="(member, idx) in teamPerformance"
                                    :key="member.name"
                                    class="group transition-colors hover:bg-card/50"
                                    :class="[
                                        idx === 0
                                ...
                                >
                                    ...
                                    >
                                        <div
                                            class="text-2xs flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-sidebar-border/50 font-bold shadow-2xs"
                                            :class="member.bg"
                                        >
                                            {{ member.avatar }}
                                        </div>
                                        <div>
                                            ...
                                            >
                                                ...
                                                <Badge
                                                    v-if="idx === 0"
                                                    variant="secondary"
                                                    class="text-3xs scale-90 bg-amber-500/15 px-1 py-0 leading-none font-bold text-amber-600 hover:bg-amber-500/15 dark:text-amber-500"
                                                >
                                                    TOP AE
                                                </Badge>
                                            </div>
                                            <div
                                                class="text-3xs font-medium text-muted-foreground"
                                            >
                                                {{ member.role }}
                                            </div>
                                        </div>
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        ...
                                        >
                                            ...
                                            >
                                            <div
                                                class="hidden h-1.5 w-12 overflow-hidden rounded-full bg-muted/65 sm:block"
                                            >
                                                <div
                                    ...
                                    <td
                                        class="px-4 py-3 text-right font-bold text-foreground"
                                    >
            ...
            <div class="flex flex-col gap-6 lg:col-span-1">
                ...
                >
                    ...
                    <CardContent
                        ...
                        v-if="assignmentInsights"
                    >
                        <!-- Recommendation Alert -->
                        <div
                            class="flex items-start gap-2.5 rounded-lg border border-primary/20 bg-primary/3 p-3"
                        >
                            <UserCheck
                            ...
                            <div class="space-y-1">
                                <div
                                    class="text-2xs font-bold tracking-wider text-primary uppercase"
                                >
                                ...
                                >
                                    Assign next lead to
                                    {{ assignmentInsights.recommended.name }}
                                </div>
                                <p
                                    class="text-3xs leading-relaxed text-muted-foreground"
                                >
                                    Selected due to lowest current workload ({{
                                        assignmentInsights.recommended
                                            .activeLeads
                                    }}
                                    active leads) and strong closing metrics ({{
                                        assignmentInsights.recommended
                                            .conversionRate
                                    }}% win rate).
                                </p>
                        ...
                        <!-- Stats Row -->
                        <div
                            class="text-2xs grid grid-cols-2 gap-2 border-t border-sidebar-border/60 pt-3"
                        >
                            <div class="space-y-0.5">
                                <span
                                    class="font-semibold text-muted-foreground"
                                    >Lowest Workload</span
                                >
                                <div class="truncate font-bold text-foreground">
                                    {{ assignmentInsights.lowestWorkload.name }}
                                </div>
                                <div
                                    class="text-3xs font-medium text-muted-foreground"
                                >
                                    Workload:
                                    {{
                                        assignmentInsights.lowestWorkload
                                            .activeLeads
                                    }}
                                    active
                                </div>
                            ...
                            <div class="space-y-0.5">
                                ...
                                <div class="truncate font-bold text-foreground">
                                    {{
                                        assignmentInsights.highestConversion
                                            .name
                                    }}
                                </div>
                                <div
                ...
                <Card
                    class="flex flex-1 flex-col border border-sidebar-border/70 bg-card/25 shadow-sm dark:border-sidebar-border/80"
                >
                    <CardHeader class="pb-2">
                        <CardTitle
                            class="flex items-center gap-1.5 text-sm font-semibold"
                    ...
                    <CardContent class="relative flex-1 pt-2">
                        <div
                            class="relative my-2 ml-1.5 space-y-4 border-l border-sidebar-border/70 pl-4 dark:border-sidebar-border"
                        >
                            <div
                                v-for="act in recentActivities"
                                :key="act.id"
                                class="text-2xs relative space-y-0.5"
                            >
                                <!-- Circle timeline node -->
                                <span
                                    class="absolute top-0.5 -left-6.5 flex h-3 w-3 scale-90 items-center justify-center rounded-full border-2 border-background"
                                ...
                                >
                                    ...
                                    <span
                                        class="text-3xs shrink-0 font-medium text-muted-foreground"
                                        >{{ act.relativeTime }}</span
                                    >
                                </div>
                                <p
                                    class="text-3xs leading-normal font-medium text-muted-foreground"
                                >
                                    {{ act.desc }}
                                </p>
                            </div>
                            ...
                            <div
                                ...
                                class="text-2xs py-6 text-center font-medium text-muted-foreground"
                            >
